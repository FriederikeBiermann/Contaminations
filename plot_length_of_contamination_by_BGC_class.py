...
df["length_contamination"] = (df["end_pos"] - df["start_pos"])/1000
...
# Determine length_contamination bins
# Determine logarithmic bins for length_contamination
min_bin = np.log10(df["length_contamination"].min())
max_bin = np.log10(df["length_contamination"].max())
length_contamination_bins = np.logspace(min_bin, max_bin, 30)
...
for category in df["Category"].unique():
    category_data = df[df["Category"] == category]["length_contamination"]
...
ax.hist(
    hist_data,
    bins=length_contamination_bins,
    label=labels,
    color=colors,
    alpha=0.8,
    stacked=True,
...
plt.xlabel("Contamination length in kb")
plt.ylabel("Counts")
plt.title("Histogram of contamination length by NP_class")
plt.xscale("log")
# Create a legend
plt.legend()

# Save the plot
plt.savefig("length_contamination_histogram_by_category.svg")
