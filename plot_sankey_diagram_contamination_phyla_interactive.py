...
# Create mappings for Host_Phylum and Contamination_Class to numerical ids
host_phylum_mapping = {
    phylum: i for i, phylum in enumerate(counts["Host_Phylum"].unique())
}
contam_class_mapping = {
    cls: i + len(host_phylum_mapping)
    for i, cls in enumerate(counts["Contamination_Class"].unique())
}
...
target = [contam_class_mapping[cls] for cls in counts["Contamination_Class"]]
...
complete_entries = counts["counts"].sum()
spacer = 0.008
height = 1 - (len(counts["Contamination_Class"].unique()) * spacer)
original_order = {key: "" for key in counts["Contamination_Class"].unique()}
for phylum in counts["Contamination_Phylum"].unique():
    ...
    for cls in phylum_class:
        ...
        cls_entries = counts[
            ...
            & (counts["Contamination_Class"] == cls)
        ]["counts"].sum()
...
node_colors = [
    ...
    phylum_colors.get(
        df[df["Contamination_Class"] == cls]["Contamination_Phylum"].iloc[0], "grey"
    )
    for cls in counts["Contamination_Class"].unique()
]
